...
from receipts where plotcode=$plotcode and pcode='$pcode' and trans_date<'$date1' and canx !='1'";
...
and trans_date between '$date1' and '$date2' and canx !='1' and remitted !='1'";
...
and pcode IN ('DEP','EDP','WDP') and canx !='1' and remitted !='1' and trans_date between '$date1' and '$date2'";
...
$exp1 = "select sum(amount) as amount from disbursement where plotcode='$plotcode' and trans_date between '$date1' and '$date2' group by plotcode";
$exp2 = $conn->query($exp1);
$exp3 =$exp2->fetch_assoc();
$disburse1 = $exp3['amount'];
$disburse = number_format($disburse1,2);

$adv1 = "select sum(amount) as amount from advance where plotcode='$plotcode' and trans_date between '$date1' and '$date2' group by plotcode";
$adv2 = $conn->query($adv1);
$adv3 =$adv2->fetch_assoc();
$advance1 = $adv3['amount'];
$advance = number_format($advance1,2);

$bnk1 = "select sum(amount) as amount from bankexp where plotcode='$plotcode' and trans_date between '$date1' and '$date2' group by plotcode";
$bnk2 = $conn->query($bnk1);
$bnk3 =$bnk2->fetch_assoc();
$bankexp1 = $bnk3['amount'];
$bankexp = number_format($bankexp1,2);

echo "<table><tr class='tr1'><th>Unit No.</th><th>Tenant Name</th><th class='right'>Deposits</th><th class='right'>Arrears</th><th class='right'>Month Bill</th>
...
while($rowz = $sql2->fetch_assoc())
{
	...
	$deposit = number_format($rowz['deposit'],2);
	$bcf = number_format($rowz['bcf'],2);
echo "<tr class='tr1'><td>$housecode</td><td>$tenant</td><td class='right'>$deposit</td><td class='right'>$arrears</td><td class='right'>$amtdue</td>
...
$row = $sql4->fetch_assoc();
	...
	$zrent = $row['paid'];
	$zdeposit = number_format($row['deposit'],2);
	$zbcf = number_format($row['bcf'],2);
	$ztotpaid1 = $row['paid']+$row['deposit'];
	$ztotpaid = number_format($row['paid']+$row['deposit'],2);
	
echo "<tr class='tr1'><th colspan=2>Totals...</th><th class='right'>$zdeposit</th><th class='right'>$zarrears</th><th class='right'>$zamtdue</th>
...
$totdeduct1 = $disburse1+$mgtfee1+$vat+$advance1+$bankexp1;
$totdeduct = number_format($totdeduct1,2);
$znetpay1 = $ztotpaid1-$totdeduct1;
$znetpay = number_format($znetpay1,2);
$mgtvat = number_format($vat,2);
echo "<table>";
...
echo "<tr><th> Total Deposi Collected</th>";
echo "<td class='right'> $zdeposit </td></tr>";
echo "<tr><th> Total Collection</th>";
echo "<td class='right'> $ztotpaid </td></tr>";
...
echo "<tr><th>Advance to LLord</th>";
echo "<td class='right'>$advance</td></tr>";
echo "<tr><th>Bank Charges</th>";
echo "<td class='right'>$bankexp</td></tr>";
echo "<tr><th>Disbursements</th>";
echo "<td class='right'>$disburse</td></tr>";
echo "<tr><th>Total Deductions</th>";
echo "<td class='right'>$totdeduct</td></tr>";
...
if ($remit=='1')
{
	$rem1 = "insert into remittance(rdate,plotcode,plotname,lordcode,pcode,nmonth,nyear,gross,disburse,amount,chequeno,mgtfee,vat,clerk,statype) 
	values('$date2','$plotcode','$plotname','$lordcode','$pcode','$nmonth','$nyear','$ztotpaid1','$disburse1','$znetpay1','$chqno',
	'$mgtfee1','$vat','$clerk','$statype')";
	$rem2 = $conn->query($rem1);
	
	$rem1 = "update receipts set remitted='1', dremit='$date2' where plotcode='$plotcode' and canx!='1' and pcode='$pcode'
	and remitted='0' and trans_date between '$date1' and '$date2'";
	$rem2 = $conn->query($rem1);
	
	$adj1 ="update adjustments set remitted='1', dremit='$date2'  where plotcode='$plotcode' and trans_date between '$date1' and '$date2'
	AND pcode ='$pcode' and remitted='0'";
	$adj2 = $conn->query($adj1);
	
	$disbur1 ="update disbursement set remitted='1', dremit='$date2'  where plotcode='$plotcode' 
	and trans_date between '$date1' and '$date2' and remitted='0'";
	$disbur2 = $conn->query($disbur1);
	
	$adv1 ="update advance set remitted='1', dremit='$date2'  where plotcode='$plotcode' and trans_date between '$date1' and '$date2' and remitted='0'";
	$adv2 = $conn->query($adv1);
	
	$bnk1 ="update bankexp set remitted='1', dremit='$date2'  where plotcode='$plotcode' and trans_date between '$date1' and '$date2' and remitted='0'";
	$bnk2 = $conn->query($bnk1);
}
?>