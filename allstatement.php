...
<div class='middle1'>
<h4 class='header'>Generate Tenant Combined Statement</h4>
<div id='msg'></div>
<div class='mypara'>
<form method='POST' action='allstatpdf.php' autocomplete='off'>
...
<th>Start Date</th>
<td><input type='text' name='date1' id='date1' value='<?php echo $date1;?>'></td>
<th>End Date</th>
<td><input type='text' name='date2' id='date2' value='<?php echo $date2;?>' ></td>
</tr>
<tr>
<th></th>
...
function generate() {
	...
	//alert(plotcode);
	var frmdata ='plotcode='+plotcode + '&date1='+ date1 + '&date2=' +date2+'&check='+check+'&tenantcode='+tenantcode;
	//alert(frmdata);
	if (plotcode==''){
	...
	}else
	{
		$.ajax({
		 type: "POST",
		 url: "genallstat.php", //call storeemdata.php to store form data
		 data: frmdata,
		 cache: false,
		 dataType: "text",
