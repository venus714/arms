...
$pay1 = "select pcode,description from receivables";
...
<html>
<head>
<title>Generate Meter Bills</title>
<meta charset="utf-8">
...
<div class='middle'>
<h4 class='header'>Generate Meter Bills</h4>
<div id='msg'></div>
<div class='mypara'>
<form method='POST' action='invpdf.php' autocomplete='off'>
...
<td class='th'><select name='pcode' id='pcode' style='width: 150px; height: 25px'>
<?php
while ($row1= $pay2->fetch_assoc())
{
	$pcode= $row1['pcode'];
...
<td class='th'><input type='text' name='nmonth' id='nmonth' value='<?php echo $nmonth;?>' style='width: 80px;'></td>
<th class='th'>Year</th>
<td class='th'><input type='text' name='nyear' id='nyear' value='<?php echo $nyear;?>' style='width: 80px;' ></td>
</tr>
</table>
<br><br>
<div class='hz'>
...
function generate() {
	...
	var frmdata ='plotcode='+plotcode +'&pcode='+pcode + '&nmonth='+nmonth + '&nyear=' +nyear+'&check='+check+'&tenantcode='+tenantcode;
	//alert(frmdata)
	...
	}else
	{
		$.ajax({
		 type: "POST",
		 url: "genmeterinv.php", //call storeemdata.php to store form data
		 data: frmdata,
		 cache: false,
		 dataType: "text",
