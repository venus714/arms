<?php
	...
	<hr>
	<h4 style="text-align: center">Combined statement<h4>';
	$html.='<p>Tenant Name: '.$tenant.'</p>
	<p>Unit Number: '.$housecode.'</p>
	<p>Property Name: '.$plotname.'</p>';
	$qr1 = "select trans_date,refno,descript,amtdue,paid,pcode,amtdue+balcf-paid as balcf from alltenantstat where tenantcode='$tenantcode' order by trans_date";
	$qr2 = $conn->query($qr1);
	$html.='<table>
	<tr><th class="hz1">Date</th><th class="hz1">Narrative</th><th class="hz1">Ref No.</th><th class="hz1">Pay Code</th><th class="hz1">Debit</th><th class="hz1">Credit</th><th class="hz1">Balance</th></tr>';
	$bal=0;
	while ($row=$qr2->fetch_assoc())
	{
		...
		$refno = $row['refno'];
		$pcode = $row['pcode'];
		$descript1 = strtolower($row['descript']);
	...
	$html.='<tr><td class="hz1">'.$trans_date.'</td><td class="hz1">'.$descript.'</td><td class="hz1">'.$refno.'</td><td class="hz1">'.$pcode.'</td> 
	<td class="right">'.$debit.'</td><td class="right">'.$credit.'</td><td class="right">'.$balcf.'</td></tr>';
	$bal = $balcf1;}
	$html.='<tr><td class="hz1" colspan=6>Balance CF<td class="right">'.$zbalcf.'</td></tr>';
...
$dompdf->render();
$dompdf->stream('comstat',array("attachement"=>0));
?>