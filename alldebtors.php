...
<form method='post' action='alldebtorspdf.php'>
<table class='tb'>
<tr>
<th class='th'>As at Date</th>
<td class='td'><input type='date' name='date1' id='date1' value='<?php echo $mdate;?>'></td></tr>

<tr><th class='th'>One Property</th><td class='td'><input type='checkbox' name='chk1' id='chk1' class='chkbox' onclick='enableprop()'></td>
<th class='th'>Property</th>
<td class='td'><select name='plotcode' id='plotcode' style='width: 125px;height: 25px;' disabled>
<?php 
...
function debtlist()
{
	...
	frmdata ='date1='+date1 +'&plotcode='+plotcode +'&check='+check;
	//alert(frmdata);
	if (date1=='')
	{
		alert('Pick a Date');
	...
	}else
	{
		$.ajax({
		 type: "POST",
		 url: "genalldebtors.php", //call storeemdata.php to store form data
		 data: frmdata,
		 cache: false,
		 dataType: "text",
