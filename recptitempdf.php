<?php
	...
	$itemname = $conn->query($item1);
	$zow = $itemname->fetch_assoc();
	$itemname = $zow['description'];
	...
	<hr><br><br>
	<p class="txt1">Payments for '.$itemname.' Posted betweem  '.$date1.' and '.$date2.'</p><br><br>
	<table><thead style="display: table-header-group">';
	
	
	$html.='<tr class="summary"><th>Date Recpted</th><th>Unit No</th><th class="left">Property</th>
	<th class="left">Tenant</th><th class="left">Recpt No</th><th class="right">Paid</th> </tr></thead>';
	...
	<td class="left">'.$tenant.'</td><td class="left">'.$recptno.'</td><td class="right">'.$paid.'</td></tr>';
	}
	$zow = $qry4->fetch_assoc();
	$zpaid= number_format($zow['paid'],2);
	$html.='<tr class="summary"><th colspan=5>Total Paid</th><th class="right">'.$zpaid.'</th></tr>
	</table>
	</body></html>';
	...
	$dompdf->render();
	$dompdf->stream('recptitem',array("attachement"=>0));
	?>